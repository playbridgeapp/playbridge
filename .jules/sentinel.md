...
## 2026-07-22 - Apple TV Insecure Token Storage
**Vulnerability:** The Apple TV app (tvOS) stored authentication bearer tokens in plaintext within `UserDefaults` (SEC-004), exposing them to compromise from physical access or iCloud backup restoration.
**Learning:** Ordinary user preference stores on Apple platforms (like `UserDefaults`) are not secure enough for raw bearer tokens. The architecture relies on these long-lived bearer tokens to re-authenticate senders. A compromise of the token allows full unauthorized remote control of the TV playback over the local network.
**Prevention:** Always use the Keychain for plaintext secrets, or store irreversible hashes (like SHA-256 verifiers) if the token must be kept in preferences. To migrate existing tokens safely without double-hashing, separate legacy plaintext token sets and new hashed verifier sets in preferences, checking incoming auth against both and migrating on successful use.
