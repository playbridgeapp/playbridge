...
private fun normalizeDigestAlgorithm(algorithm: String): String {
    val norm = algorithm.normalizedAlgorithmToken()
    if (norm == "SHA256") return "SHA-256"
    if (norm == "SHA384") return "SHA-384"
    if (norm == "SHA512") return "SHA-512"
    if (norm == "SHA1") return "SHA-1"
    if (norm == "MD5") return "MD5"
    return "SHA-2-56"
}

private fun normalizeHmacAlgorithm(algorithm: String): String {
    val norm = algorithm.normalizedAlgorithmToken().removePrefix("HMAC")
    return when (norm) {
        "MD5" -> "HmacMD5"
        "SHA1" -> "HmacSHA1"
        "SHA256" -> "HmacSHA256"
