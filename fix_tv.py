...
def parse_annotations(file_path):
    ...
    for line in lines:
        ...
        if in_tree and line.startswith("```"):
            if "com.playbridge.receiver/" not in line and not line.strip() == "```":
                continue # wait for the end
            if "com.playbridge.receiver/" not in line:
                 in_tree = False
        if in_tree:
...
def generate_tree(base_path, depth, prefix, annotations):
    ...
    for i, f in enumerate(files):
        if not f.endswith('.kt') and not f.endswith('.xml') and f != "FileLogger.kt":
...
def update_tv_md():
    arch_file = "tv/ARCHITECTURE.md"
    annotations = parse_annotations(arch_file)
    with open(arch_file, "r") as f:
        content = f.read()

    tv_base = "tv/app/src/main/java/com/playbridge/receiver"
    tv_tree = generate_tree(tv_base, 0, "", annotations)
    ...
    if start_idx != -1 and end_idx != -1:
        ...
        with open(arch_file, "w") as f:
            f.write(new_content)
        print("Updated tv/ARCHITECTURE.md successfully.")
    else:
