...
- **Browser** — `tv/android/browser` (`com.playbridge.browser`). Android TV web browser (GeckoView) with a bundled uBlock Origin, designed for D-pad navigation.
...
./gradlew :browser:app:assembleDebug
```

Gradle root project: **PlayBridgeTV** (`tv/android/settings.gradle.kts`, modules `:player:app`, `:browser:app`, `:shared`).
