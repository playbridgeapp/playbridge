...
class PrePlayActivity : ComponentActivity() {
    ...
    private fun launchPlayer(stream: ScoredStremioStream) {
        ...
        FileLogger.i(TAG, "Launching player for stream: ${stream.name}")
        FileLogger.i(TAG, "  URL: ${stream.url}")
        FileLogger.i(TAG, "  Score: ${stream.score}")
        ...
        if (finalMode == "external" || finalMode == "external_mpv") {
            // External players don't support SeriesContext easily via Intent
            // Handle as simple Command.Play
