...
- `patches/audio-record-factory.patch`
  - Adds an optional Java `AudioRecordFactory` to `JavaAudioDeviceModule`.
  - Lets PlayBridge supply an Android playback-capture `AudioRecord` without
    opening or transmitting the microphone.
  - Does not change JNI signatures and therefore does not require rebuilt native
    WebRTC libraries.
