...
class VideoFilterManager {
    ...
    fun applyFilter(filter: VideoFilter) {
        ...
        val matrix = VideoFilterAndroid.matrixFor(filter)
    ...
    fun applyCustom(brightness: Float, contrast: Float, saturation: Float) {
        ...
        val matrix = VideoFilterAndroid.customMatrix(brightness, contrast, saturation)
        applyMatrixToPlayer(matrix)
    }
    ...
    fun reapplyFilter() {
        ...
        if (currentFilter == VideoFilter.CUSTOM) {
            val matrix = VideoFilterAndroid.customMatrix(customBrightness, customContrast, customSaturation)
            applyMatrixToPlayer(matrix)
        } else {
            val matrix = VideoFilterAndroid.matrixFor(currentFilter)
            applyMatrixToPlayer(matrix)
        }
    }
