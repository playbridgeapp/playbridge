...
impl LocalMediaServer {
    pub async fn start(file_path: PathBuf) -> Result<(Self, tokio::task::JoinHandle<()>), String> {
        ...
        let lan_ip = get_local_lan_ip()
            .ok_or_else(|| "Could not determine local LAN IP address".to_string())?;
        ...
        getrandom::fill(&mut token).map_err(|e| format!("Failed to generate media token: {e}"))?;
        let token = token
            .iter()
            .map(|byte| format!("{byte:02x}"))
            .collect::<String>();
        let route_path = format!("/media/{token}");
        let url = format!("http://{lan_ip}:{port}{route_path}");

        let serve_file = ServeFile::new(&file_path);
        let app = Router::new().route_service(&route_path, serve_file);
